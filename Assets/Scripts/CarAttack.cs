...
public class CarAttack : MonoBehaviour
{
    ...
    public void DetectCollistion(){
        ...
        foreach (var el in hitColliders)
        {
            ...
            (gameObject.CompareTag("Enemy") && el.gameObject.CompareTag("Player"))){
                if(gameObject.CompareTag("Enemy"))
                    GetComponent<NavMeshAgent>().SetDestination(el.transform.position); // Заставляем вражескую машину двигаться на нас
                if(_coroutine == null)
    ...
    internal IEnumerator StartAttack(Collider enemyPos){
